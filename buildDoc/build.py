...
# for testing, when links dont matter
QUICKBUILD_MODULES = [
                      base,
                      chord,
                      note,
...
class PartitionedClass(PartitionedName):
    ...
    def getElement(self, partName):
        ...
        Attribute(name='_getDiatonicNoteNum', kind='method', defining_class=<class 'music21.pitch.Pitch'>, object=<function _getDiatonicNoteNum at 0x...>)
    ...
    def getDoc(self, partName):
        ...
        >>> from music21 import meter
        >>> c = PartitionedClass(meter.MeterTerminal)
        >>> "\n        >>> a = meter.MeterTerminal('3/4')" in c.getDoc('subdivideByCount')
        True
        '''
        element = self.getElement(partName)
    ...
    def getNames(self, nameKind, mroIndex=None, public=True, getInit=True,
        ...
        ['classSortOrder', 'isSpanner', 'isStream', 'isVariant', 'hideObjectOnPrint', 'xPosition', 'sites', 'groups', 'id']
        ...
        ['isSpanner', 'isStream', 'isVariant', 'xPosition', 'id', 'sites', 'groups', 'hideObjectOnPrint']
...
#                sub.append(lineSrc + '\n')
        ...
        codeIndent = '    ' # additional indent for code sections...
...
#                        msg.append(indent + u'\u2060 \n') # can add indent here
                ...
                wordJoinerAdded = False
                if addWordJoinerToPreviousLine == True:
                    msg[-1] = indent + codeIndent + u'\u2060 \n'
                    addWordJoinerToPreviousLine = False
                    wordJoinerAdded = True
                
                if inExamples == False:
                    space = '\n\n'
                    if wordJoinerAdded is not True:
                        msg.append(space + indent + '.. code:: python' + space)
                    
                    # if import is the module import, replace with package
                    if line.startswith(docTestImportPackage):
                        # can try to mark up import here
                        msg.append(space + indent + codeIndent + docTestImportPackageReplace + '\n')
                    else: # if no import is given, assume we need a mod import
                        # need only one return after first line
                        msg.append(space + indent + codeIndent + line + '\n')
                    inExamples = True
                else:
                    msg.append(indent + codeIndent + line + '\n')
            # images should be at the same indent level
                ...
                if addWordJoinerToPreviousLine == True: # nope didnt need one
                    addWordJoinerToPreviousLine = False
                    inExamples = False
                    msg.append(indent + line + '\n')
                elif inExamples is False:
                    msg.append(indent + line + '\n')
                else:
                    msg.append(indent + codeIndent + line + '\n')
        msg.append('\n')
...
class Documentation(RestructuredWriter):
    ...
    def __init__(self):
        ...
        self.chaptersMain = ['usersGuide_00_introduction',
...
if __name__ == '__main__':
    ...
    if buildDoc:
        for fmt in formatList:
            a = Documentation()
            if 'quick' in sys.argv: # be sure to clean out rst of all module*.rst files too
                a.modulesToBuild = QUICKBUILD_MODULES # just 4 modules for testing
            a.main(fmt)
