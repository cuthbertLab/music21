...
class StaticFileCopier(DocumentationWriter):
    ...
    '''
    def run(self):
        excludedFiles = ['.ipynb', '__pycache__', '.pyc',
                         '.gitignore', 'conf.py', '.DS_Store']
        for subPath in sorted(self.docSourcePath.rglob('*')):
            if '.ipynb_checkpoints' in subPath.parts:
                continue
...
class JupyterNotebookReSTWriter(ReSTWriter):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.jupyterNotebookFilePaths = sorted(list(JupyterNotebookIterator()))
