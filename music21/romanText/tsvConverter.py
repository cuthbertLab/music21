...
# Purpose:      Convertor for the DCML lab's tabular format for representing harmonic analysis.
#
# Authors:      Mark Gotham
#
...
class TsvHandler:
    ...
    def toM21Stream(self, write=False):
        '''
        ...
        if write==True:
            try:
                ...
                title = 'Op'+scl0.op+'_No'+scl0.no+'_Mov'+scl0.mov
            except:
                title='music21Stream'
            s.write(fmt='musicxml', fp='./'+title+'.xml')
...
class M21toTSV:
    ...
    def m21ToTsv(self):
        '''
        Writes a list of music21 chords to a list of lists
...
class Test(unittest.TestCase):
    ...
    def testTsvHandler(self):
        ...
        importedTsv = TsvHandler('tsvEgOp18No1Mov3.tsv')
        ...
        #(write=True) Tested separately, including with a fake time signature change;
        # TODO: formal test of write without actually writing
        self.assertEqual(strm.parts[0].measure(1)[0].figure, 'I') # First item in measure 1

    def testM21ToTsv(self):

        from music21 import converter
        bachHarmony = converter.parse('choraleExample.rntxt', format='romanText')
        initial = M21toTSV(bachHarmony)
        # TODO: again, test initial.write()
