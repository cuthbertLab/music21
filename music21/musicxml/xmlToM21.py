...
class MeasureParser(XMLParserBase):
    ...
    def xmlToNote(self, mxNote):
        ...
        if isChord is True: # and isRest is False...?            
            ...
            try:
                ...
                raise MusicXMLImportException('cannot translate note in measure {0}: {1}'.format(
    ...
    def xmlToPitch(self, mxNote, inputM21=None):
        ...
        >>> b = EL('<note><pitch><step>E</step><alter>-1</alter><octave>3</octave></pitch></note>')
        >>> a = MP.xmlToPitch(b)
        >>> print(a)
        E-3
        ...
        if mxAccidentalName is not None:
            ...
            try:
                ...
                raise MusicXMLImportException('incorrect accidental {0} for pitch {1}' .format(
                                                                                accAlter, p))
...
#             mxTiedList = mxNotations.getTieds()
    ...
    def xmlToTuplets(self, mxNote):
        ...
        returnTuplets = []
        # returnTupletDict = {} # use this to map number back to itself so 
        # that startStop can be found
        ...
        for mxTuplet in mxTuplets:
            ...
            if mxTupletActual is None or mxTupletNormal is None:
                t = timeModTup # in theory either can be absent, but so far I have only seen both.
            else:
                t = duration.Tuplet()
            ...
            # number is not needed, since each tuplet will create a new number.
...
if __name__ == '__main__':
    import music21
    music21.mainTest(Test) #, runTest='testComplexTupletNote')
