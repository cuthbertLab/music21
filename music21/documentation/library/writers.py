...
class ReSTWriter(object):
    ...
    def sourceToAutogenerated(self, sourcePath):
        ...
        sets up any directories necessary along the way...
        '''
        outputPath = sourcePath.replace(self.docSourcePath, self.docGeneratedPath)
        dirForIterating = os.path.abspath(
                os.path.dirname(outputPath),
            )
        while self.docGeneratedPath in dirForIterating:
            self.setupOutputDirectory(dirForIterating)
            dirForIterating = os.path.abspath(
                os.path.dirname(dirForIterating),
            )
        return outputPath
    ...
    def setupOutputDirectory(self, outputDirectory=None):
        '''
        creates outputDirectory if it doesnt exist.
...
class IPythonNotebookReSTWriter(ReSTWriter):
    ...
    def runNBConvert(self, ipythonNotebookFilePath):
        ...
        outputPath = self.sourceToAutogenerated(ipythonNotebookFilePath)
