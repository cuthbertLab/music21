...
function run() {
    ...
    var file   = new QFile(pluginPath + "/music21interface.ui");
...
function accept() {
    var cmd = form.command.plainText;
    var cmdstr = "";
    cmdstr += cmd;
    ...
    data = new QByteArray();
    data.append("xmltext=" + content+"&cmd="+cmdstr+"&appname=musescore")
...
var mscorePlugin =
{
   majorVersion: 1,
   minorVersion: 1,
   menu: 'Plugins.Music21 (Old).Advanced Interface',
   init: init,
   run: run,
   onClose: close
};
