...
CHORD_TYPES = collections.OrderedDict([
    ...
    ('half-diminished-seventh', ['1,-3,-5,-7', ['m7b5', '/o7']]),
    # Y: 'half-diminished'
    ...
    ('dominant-minor-ninth-flat-fifth', ['1,3,-5,-7,-9', ['7b9b5']]),  # Y
    ...
    for chordKind in CHORD_TYPES:
        ...
        if common.isListLike(chordDegrees):
            if len(chordDegrees) == 2 and isTriad:
                ...
                if compare((d3, d5, d7, d9, d11), chordDegrees, permittedOmitions=(5,)):
                    kind = chordKind
                    kindStr = chordKindStr[0]
...
class ChordSymbol(Harmony):
    ...
    def transpose(self, value, *, inPlace=False):
        if inPlace:
            root = self.root()
            super(ChordSymbol, self).transpose(value=value,
                                               inPlace=inPlace)
            self.root(root.transpose(value=value, inPlace=False))
            self = chordSymbolFromChord(self)
            return None
        else:
            root = self.root()
            transposition = super(ChordSymbol, self).transpose(
                value=value,
                inPlace=inPlace)
            transposition.root(root.transpose(value=value, inPlace=False))
            transposition = chordSymbolFromChord(
                transposition)
