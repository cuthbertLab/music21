...
def example5_6():
    ...
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example5_6())
    >>> inBraille[0] == "⠼⠁⠀⠐⠮⠽⠾⠀⠝⠞⠮⠞⠝⠀⠾⠍⠘⠉⠍⠀⠷⠾⠎⠗⠀⠮⠘⠉⠮⠍⠣⠅⠀⠀⠀⠀⠀⠀⠀⠀"
    True
    '''
...
def example6_1():
    '''
    >>> from music21.braille import translate
    >>> translate.noteToBraille(note.Note('C#4', quarterLength = 2.0), showOctave = False) == "⠩⠝"
    True
    >>> translate.noteToBraille(note.Note('Gn4', quarterLength = 2.0), showOctave = False) == "⠡⠗"
    True
    >>> translate.noteToBraille(note.Note('E-4', quarterLength = 2.0), showOctave = False) == "⠣⠏"
    True
    '''
    pass
...
def example6_2():
    '''
    >>> from music21.braille import translate
    >>> translate.measureToBraille(example6_2()[0], showLeadingOctave = False) == "⠩⠳⠩⠩⠻⠗"
    True
    '''
...
def example6_3():
    '''
    >>> from music21.braille import translate
    >>> translate.measureToBraille(example6_3()[0], showLeadingOctave = False) == "⠝⠣⠞⠈⠉"
    True
    >>> translate.measureToBraille(example6_3()[1], showLeadingOctave = False) == "⠺⠹⠪⠻"
    True
    '''
...
def example6_4():
    '''
    >>> from music21.braille import translate
    >>> translate.measureToBraille(example6_4()[0], showLeadingOctave = False) == "⠡⠫⠋⠊⠙⠡⠋"
    True
    >>> translate.measureToBraille(example6_4()[1], showLeadingOctave = False) == "⠟⠄"
    True
    '''
...
def example6_5():
    '''
    >>> from music21.braille import translate
    >>> translate.measureToBraille(example6_5()[0], showLeadingOctave = False) == "⠙⠣⠚⠊⠓⠻"
    True
    >>> translate.measureToBraille(example6_5()[1], showLeadingOctave = False) == "⠓⠡⠚⠹⠱"
    True
    '''
...
def example6_6():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example6_6())
    >>> inBraille[0] == "⠼⠁⠀⠐⠹⠩⠹⠱⠩⠱⠀⠫⠻⠩⠻⠳⠀⠩⠳⠪⠣⠺⠡⠺⠀⠝⠥⠀⠏⠣⠫⠱⠀⠹⠱⠡⠫⠹⠀⠀"
    True
    >>> inBraille[1] == "⠀⠀⠐⠺⠣⠺⠪⠡⠺⠀⠝⠥⠣⠅⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
...
def example6_7():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example6_7())
    >>> inBraille[0] == "⠼⠁⠀⠨⠳⠩⠻⠡⠻⠫⠀⠣⠫⠱⠣⠱⠹⠀⠺⠣⠺⠪⠣⠪⠀⠳⠩⠻⠡⠻⠫⠀⠣⠫⠱⠣⠱⠹⠀⠀"
    True
    >>> inBraille[1] == "⠀⠀⠸⠺⠹⠱⠫⠀⠝⠥⠀⠽⠣⠅⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
...
def example6_8():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example6_8())
    >>> inBraille[0] == "⠼⠁⠀⠨⠹⠙⠣⠚⠣⠊⠙⠛⠣⠊⠈⠉⠀⠊⠓⠛⠓⠝⠀⠣⠱⠛⠑⠙⠙⠣⠊⠛⠀⠓⠓⠙⠙⠟⠣⠅"
    True
    '''
...
def example6_9():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example6_9())
    >>> inBraille[0] == "⠼⠁⠀⠸⠫⠗⠫⠀⠩⠻⠎⠻⠀⠓⠊⠺⠳⠫⠀⠩⠑⠋⠩⠻⠱⠳⠀⠹⠏⠩⠻⠀⠳⠞⠹⠀⠀⠀⠀⠀"
    True
    >>> inBraille[1] == "⠀⠀⠸⠚⠊⠓⠚⠊⠓⠩⠛⠩⠑⠀⠯⠣⠅⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
...
def example6_10():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example6_10())
    >>> inBraille[0] == "⠼⠁⠀⠐⠪⠎⠪⠈⠉⠀⠪⠧⠣⠞⠀⠹⠹⠧⠹⠈⠉⠀⠹⠧⠕⠀⠋⠫⠋⠻⠧⠀⠋⠫⠋⠱⠧⠀⠀⠀"
    True
    >>> inBraille[1] == "⠀⠀⠨⠙⠹⠙⠣⠚⠺⠚⠀⠎⠄⠧⠣⠅⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
...
def example6_11():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example6_11())
    >>> inBraille[0] == "⠼⠁⠀⠸⠪⠄⠚⠹⠺⠀⠊⠙⠋⠙⠊⠙⠫⠀⠱⠄⠛⠪⠻⠀⠑⠛⠊⠛⠑⠛⠪⠀⠩⠳⠄⠋⠎⠀⠀⠀"
    True
    >>> inBraille[1] == "⠀⠀⠐⠺⠄⠚⠝⠀⠑⠙⠚⠊⠩⠓⠋⠩⠛⠓⠀⠎⠈⠉⠊⠩⠓⠊⠣⠅⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
...
def example6_12():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example6_12())
    >>> inBraille[0] == "⠼⠁⠀⠨⠋⠣⠋⠑⠣⠑⠙⠊⠣⠊⠓⠀⠣⠣⠚⠣⠊⠓⠣⠓⠛⠋⠑⠙⠀⠑⠚⠣⠚⠡⠚⠙⠑⠋⠛⠀"
    True
    >>> inBraille[1] == "⠀⠀⠐⠓⠩⠓⠊⠚⠹⠧⠣⠅⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
...
def example7_1():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example7_1())
    >>> inBraille[0] == "⠼⠁⠀⠈⠽⠀⠘⠽⠀⠸⠽⠀⠐⠽⠀⠨⠽⠀⠰⠽⠀⠠⠽⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
    '''
    bm = stream.Part()
...
def example7_2():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example7_2())
    >>> inBraille[0] == "⠼⠁⠀⠈⠈⠮⠾⠀⠠⠠⠽⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
    '''
    bm = stream.Part()
...
def example7_3a():
    '''
    >>> from music21.braille import translate
    >>> translate.measureToBraille(example7_3a()[0]) == "⠐⠹⠫"
    True
    '''
...
def example7_3b():
    '''
    >>> from music21.braille import translate
    >>> translate.measureToBraille(example7_3b()[0]) == "⠨⠝⠄⠪"
    True
    '''
...
def example7_4a():
    '''
    >>> from music21.braille import translate
    >>> translate.measureToBraille(example7_4a()[0]) == "⠐⠝⠐⠎"
    True
    '''
...
def example7_4b():
    '''
    >>> from music21.braille import translate
    >>> translate.measureToBraille(example7_4b()[0]) == "⠨⠝⠐⠏"
    True
    '''
...
def example7_5a():
    '''
    >>> from music21.braille import translate
    >>> translate.measureToBraille(example7_5a()[0]) == "⠸⠝⠟"
    True
    '''
...
def example7_5b():
    '''
    >>> from music21.braille import translate
    >>> translate.measureToBraille(example7_5b()[0]) == "⠐⠟⠨⠝"
    True
    '''
...
def example7_6():
    ...
    >>> from music21.braille import translate
    >>> inPart = example7_6()
    >>> inBraille = translate.partToBraille(inPart)
    >>> inBraille[0] == "⠼⠁⠀⠣⠨⠋⠋⠋⠋⠀⠑⠑⠣⠺⠀⠙⠙⠙⠙⠀⠣⠐⠋⠨⠙⠣⠺⠀⠛⠛⠨⠹⠀⠣⠚⠚⠨⠻⠀⠀"
    True
    >>> inBraille[1] == "⠀⠀⠣⠨⠋⠑⠙⠣⠚⠀⠣⠨⠫⠣⠐⠫⠣⠅⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
...
def example7_7():
    ...
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example7_7())
    >>> inBraille[0] == "⠼⠁⠀⠐⠊⠙⠛⠙⠀⠨⠊⠙⠛⠙⠀⠰⠊⠛⠙⠑⠀⠨⠊⠛⠋⠙⠀⠟⠣⠅⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
    '''
...
def example7_8():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example7_8())
    >>> inBraille[0] == "⠼⠁⠀⠸⠝⠘⠳⠸⠫⠀⠱⠄⠙⠹⠹⠀⠸⠎⠳⠫⠀⠱⠗⠳⠀⠐⠹⠄⠚⠪⠳⠀⠪⠻⠹⠪⠀⠀⠀⠀"
    True
    >>> inBraille[1] == "⠀⠀⠘⠳⠳⠸⠱⠳⠀⠫⠝⠄⠣⠅⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
...
def example7_9():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(example7_9())
    >>> inBraille[0] == "⠼⠁⠀⠐⠫⠄⠊⠙⠋⠀⠱⠑⠙⠺⠀⠫⠄⠩⠓⠚⠨⠋⠀⠹⠙⠚⠪⠀⠊⠩⠙⠋⠊⠨⠙⠓⠀⠀⠀⠀"
    True
    >>> inBraille[1] == "⠀⠀⠨⠛⠑⠐⠊⠨⠋⠑⠐⠛⠀⠨⠙⠚⠐⠑⠊⠩⠓⠋⠀⠎⠄⠣⠅⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
...
def example8_1a():
    ...
    >>> from music21.braille import translate
    >>> translate.keySigToBraille(key.KeySignature(-1)) == "⠣"
    True
    >>> translate.keySigToBraille(key.KeySignature(-2)) == "⠣⠣"
    True
    >>> translate.keySigToBraille(key.KeySignature(-3)) == "⠣⠣⠣"
    True
    >>> translate.keySigToBraille(key.KeySignature(-4)) == "⠼⠙⠣"
    True
    >>> translate.keySigToBraille(key.KeySignature(-5)) == "⠼⠑⠣"
    True
    >>> translate.keySigToBraille(key.KeySignature(-6)) == "⠼⠋⠣"
    True
    >>> translate.keySigToBraille(key.KeySignature(-7)) == "⠼⠛⠣"
    True
    '''
    pass
...
def example8_2():
    ...
    >>> from music21.braille import translate
    >>> translate.timeSigToBraille(meter.TimeSignature('6/8')) == "⠼⠋⠦"
    True
    >>> translate.timeSigToBraille(meter.TimeSignature('2/4')) == "⠼⠃⠲"
    True
    >>> translate.timeSigToBraille(meter.TimeSignature('12/8')) == "⠼⠁⠃⠦"
    True
    >>> translate.timeSigToBraille(meter.TimeSignature('2/2')) == "⠼⠃⠆"
    True
    '''
    pass
...
def example8_5():
    ...
    >>> from music21.braille import translate
    >>> translate.timeSigToBraille(meter.TimeSignature('common')) == "⠨⠉"
    True
    >>> translate.timeSigToBraille(meter.TimeSignature('cut')) == "⠸⠉"
    True
    '''
    pass
...
def example8_6():
    ...
    >>> from music21.braille import translate
    >>> translate.keyAndTimeSigToBraille(sampleKeySig = key.KeySignature(1), sampleTimeSig = meter.TimeSignature('2/4')) == "⠩⠼⠃⠲"
    True
    >>> translate.keyAndTimeSigToBraille(sampleKeySig = key.KeySignature(-3), sampleTimeSig = meter.TimeSignature('3/4')) == "⠣⠣⠣⠼⠉⠲"
    True
    >>> translate.keyAndTimeSigToBraille(sampleKeySig = key.KeySignature(4), sampleTimeSig = meter.TimeSignature('3/8')) == "⠼⠙⠩⠼⠉⠦"
    True
    >>> translate.keyAndTimeSigToBraille(sampleKeySig = key.KeySignature(3), sampleTimeSig = meter.TimeSignature('3/8')) == "⠩⠩⠩⠼⠉⠦"
    True
    ...
    >>> translate.keyAndTimeSigToBraille(sampleKeySig = None, sampleTimeSig = meter.TimeSignature('4/4')) == "⠼⠙⠲"
    True
    >>> translate.keyAndTimeSigToBraille(sampleKeySig = key.KeySignature(0), sampleTimeSig = meter.TimeSignature('4/4')) == "⠼⠙⠲"
    True
    
    
    >>> translate.keyAndTimeSigToBraille(sampleKeySig = key.KeySignature(-1), sampleTimeSig = meter.TimeSignature('3/4')) == "⠣⠼⠉⠲"
    True
    >>> translate.keyAndTimeSigToBraille(sampleKeySig = key.KeySignature(0), sampleTimeSig = meter.TimeSignature('6/8')) == "⠼⠋⠦"
    True
    '''
    pass
...
def example8_7a():
    ...
    >>> from music21 import meter
    >>> from music21 import tempo
    >>> translate.headingToBraille(sampleTempoText = tempo.TempoText("Andante"), sampleKeySig = key.KeySignature(-4),\
    sampleTimeSig = meter.TimeSignature("4/4"), sampleMetronomeMark = None) == "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠠⠁⠝⠙⠁⠝⠞⠑⠲⠀⠼⠙⠣⠼⠙⠲⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
    >>> translate.headingToBraille(sampleTempoText = tempo.TempoText("Con moto"), sampleKeySig = key.KeySignature(3),\
    sampleTimeSig = meter.TimeSignature("3/8"), sampleMetronomeMark = None) == "⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠠⠉⠕⠝⠀⠍⠕⠞⠕⠲⠀⠩⠩⠩⠼⠉⠦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
    >>> translate.headingToBraille(sampleTempoText = tempo.TempoText("Andante cantabile"), sampleKeySig = None,\
    sampleTimeSig = meter.TimeSignature("4/4"), sampleMetronomeMark = None) == "⠀⠀⠀⠀⠀⠀⠀⠀⠠⠁⠝⠙⠁⠝⠞⠑⠀⠉⠁⠝⠞⠁⠃⠊⠇⠑⠲⠀⠼⠙⠲⠀⠀⠀⠀⠀⠀⠀⠀⠀"
    True
    >>> translate.headingToBraille(sampleTempoText = tempo.TempoText("Very brightly"), sampleKeySig = key.KeySignature(2),\
...
def example8_8():
    ...
    >>> translate.metronomeMarkToBraille(sampleMetronomeMark = tempo.MetronomeMark(number = 80, referent = note.HalfNote())) == "⠝⠶⠼⠓⠚"
    True
    '''
    pass
...
def example8_9():
    ...
    >>> from music21 import meter
    >>> from music21 import tempo
    >>> translate.headingToBraille(sampleTempoText = tempo.TempoText("Andante"), sampleKeySig = key.KeySignature(-3),\
    sampleTimeSig = meter.TimeSignature("12/8"), sampleMetronomeMark = tempo.MetronomeMark(number = 132, referent = note.EighthNote())) == \
    "⠀⠀⠀⠀⠀⠀⠀⠀⠠⠁⠝⠙⠁⠝⠞⠑⠲⠀⠙⠶⠼⠁⠉⠃⠀⠣⠣⠣⠼⠁⠃⠦⠀⠀⠀⠀⠀⠀⠀⠀"
    True
    >>> h2 = translate.headingToBraille(sampleTempoText = tempo.TempoText("Lento assai, cantante e tranquillo"), sampleKeySig = key.KeySignature(-5),\
...
def drill8_1():
    '''
    >>> from music21.braille import translate
    >>> inBraille = translate.partToBraille(drill8_1())
    >>> inBraille[0] == "⠀⠀⠀⠀⠀⠀⠠⠁⠝⠙⠁⠝⠞⠑⠀⠍⠁⠑⠎⠞⠕⠎⠕⠲⠀⠹⠶⠼⠊⠃⠀⠨⠉⠀⠀⠀⠀⠀⠀⠀"
    True
    >>> inBraille[1] == "⠼⠁⠀⠐⠎⠓⠛⠫⠀⠱⠫⠛⠓⠪⠀⠳⠨⠙⠚⠪⠳⠀⠻⠄⠋⠕⠀⠹⠫⠪⠨⠫⠀⠫⠱⠙⠚⠪⠀⠀"
    True
