...
class Edge(object):
    ...
    'bi'
    '''
    def __init__(self, intervalData=None, id=None, direction=DIRECTION_BI): # id is okay: @ReservedAssignment
        ...
        self._direction = direction # can be bi, ascending, descending
        self.weight = 1.0
        # store id
        self.id = id
    ...
    def addDirectedConnection(self, node1, node2, direction=None):
        ...
        if common.isStr(node2) or common.isNum(node2):
            n2Id = node2
        else: # assume an Node
            n2Id = node2.id
...
class BoundIntervalNetwork(IntervalNetwork):
    ...
    def _nodeIdToEdgeDirections(self, nId):
        ...
        ['bi']
        >>> net._nodeIdToEdgeDirections(0)
        ['bi', 'bi']
        >>> net._nodeIdToEdgeDirections(6)
        ['ascending', 'ascending']
        >>> net._nodeIdToEdgeDirections(5)
        ['descending', 'descending']
        >>> # this node has bi-directional (from below), 
        >>> # ascending (to above), and descending (from above)
        >>> # edge connections connections
        >>> net._nodeIdToEdgeDirections(3) 
    ...
    def _processAlteredNodes(self, alteredDegrees, n, p, direction):
        '''
    ...
    def nextPitch(self, pitchReference, nodeName, pitchOrigin, 
        ...
        #  'direction', direction, 'self._nodes[nodeId].degree', self._nodes[nodeId].degree, 
        ...
        if nodeId is None and getNeighbor in [True,     
            ...
            # 'getNeighbor', getNeighbor, 'source nodeId', nodeId, 'lowId/highId', lowId, highId])
        ...
        p = self.getPitchFromNodeDegree(pitchReference=pitchReference, 
            ...
            maxPitch=None, 
            alteredDegrees={} # need unaltered tone here, thus leaving out
            )

        #environLocal.printDebug(['nextPitch()', 'pitch obtained based on nodeName', 
        # nodeName, 'p', p, 'nodeId', nodeId, 'self._nodes[nodeId].degree', 
        # self._nodes[nodeId].degree])
    ...
    def _realizeDescending(self, pitchReference, nodeId=None, 
        ...
        ([], [])
        >>> (pitches, nodeKeys) = net._realizeDescending('c3', 1, 'c2')
        >>> [str(p) for p in pitches]
        ['C2', 'D2', 'E2', 'F2', 'G2', 'A2', 'B2']
        >>> nodeKeys
        ['terminusLow', 0, 1, 2, 3, 4, 5]
        >>> (pitches, nodeKeys) = net._realizeDescending('c3', 1, 'c2', includeFirst=True)
        >>> [str(p) for p in pitches]
        ['C2', 'D2', 'E2', 'F2', 'G2', 'A2', 'B2', 'C3']
        >>> nodeKeys
        ['terminusLow', 0, 1, 2, 3, 4, 5, 'terminusLow']

        >>> (pitches, nodeKeys) = net._realizeDescending('a6', 'high')
        >>> [str(p) for p in pitches]
        ['A5', 'B5', 'C#6', 'D6', 'E6', 'F#6', 'G#6']
        >>> nodeKeys
        ['terminusLow', 0, 1, 2, 3, 4, 5]

        >>> (pitches, nodeKeys) = net._realizeDescending('a6', 'high', includeFirst=True)
        >>> [str(p) for p in pitches]
        ...
        while True:
            ...
            #environLocal.printDebug(['_realizeDescending', 'appending pitch', pCollect, 'includeFirst', includeFirst])
    ...
    def realize(self, pitchReference, nodeId=None, minPitch=None, maxPitch=None, 
        ...
        >>> [str(p) for p in pitches]
        ['C2', 'D2', 'E2', 'F2', 'G2', 'A2', 'B-2', 'C3', 'D3', 'E3', 'F3', 'G3', 'A3', 'B-3', 'C4']
        >>> nodeKeys
    ...
    def realizeMinMax(self, pitchReference, nodeId=None, alteredDegrees=None):
        ...
        low, high = self.realizeTermini(pitchReference=pitchReference, 
                    nodeId=nodeId, alteredDegrees=alteredDegrees)
...
#             :width: 600
    ...
    def getRelativeNodeId(self, pitchReference, nodeName, pitchTarget, 
        ...
        if len(post) == 0:
            ...
            return common.weightedSelection(post, 
                [self._nodes[x].weight for x in post])
    ...
    def match(self, pitchReference, nodeId, pitchTarget, 
        ...
        nodesRealized = self.realizePitch(pitchReference, nodeId, 
                        minPitch, maxPitch, alteredDegrees=alteredDegrees)
...
class Test(unittest.TestCase):
    ...
    def testDirectedA(self):
        ...
        self.assertEqual(self.realizePitchOut(net.realize('a4', 1, 'a3', 'a5')), "([A3, B3, C4, D4, E4, F4, G4, A4, B4, C5, D5, E5, F#5, G#5, A5], ['terminusLow', 6, 7, 8, 9, 10, 11, 'terminusLow', 0, 1, 2, 3, 4, 5, 'terminusHigh'])")
    ...
    def testRealizeDescending(self):
        ...
        self.assertEqual(self.realizePitchOut(net._realizeDescending('c3', 'high', minPitch='c2')), 
