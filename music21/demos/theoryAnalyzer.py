        ...
        if partNumToIdentify == None:
            ...
            for vsnt in self.getVerticalSliceNTuplets(nTupletNum):
                if testFunction(vsnt, partNumToIdentify) is not False:
                    tr = VerticalSliceNTupletTheoryResult(vsnt, partNumToIdentify)
                    if editorialDictKey != None:
...
#-------------------------------------------------------------------------------
# Theory Result Objects - each of some class mirroring voiceLeading.py
# these objects are created by identify() methods above, and always
# contain the direct pointer to the voiceLeading object that caused
# the create of the result object 
...
class TheoryResult(object):
    ...
    in the score, a textual description of the error, and a value. Uses subclasses
    ...
    def markNoteEditorial(self, editorialDictKey, editorialValue, editorialMarkDict={2:[1]}):        
        ...
        for vsNum, partNumList in editorialMarkDict.items():
            for partNum in partNumList:
                self.vsnt.verticalSlices[vsNum].noteFromPart(partNum).editorial.misc[editorialDictKey] = editorialValue
