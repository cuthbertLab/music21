...
class ModalCounterpoint(object):
    ...
    def findParallelFifths(self, srcStream, cmpStream):
        ...
        >>> cp = counterpoint.counterpoint.ModalCounterpoint(stream1 = bass, stream2 = sop)
    ...
    def isParallelFifth(self, note11, note12, note21, note22):
        ...
        >>> m1 = note.Note('D4')
        >>> m2 = note.Note('F4')
        >>> cp = counterpoint.counterpoint.ModalCounterpoint()
...
##            return False
    ...
    def allValidHarmony(self, stream1, stream2):
        ...
        >>> sop = stream.Stream()
        >>> sop.append([m1, m2, m3, m4])
        >>> cp = counterpoint.counterpoint.ModalCounterpoint(stream1 = sop, stream2 = bass)
    ...
    def findAllBadFifths(self, stream1, stream2):
        ...
        >>> cp = counterpoint.counterpoint.ModalCounterpoint(s1, s2)
...
#                environLocal.printDebug([note1.name + str(note1.octave) for note1 in bottom.notes])
                ...
                if not thirdsGood2: environLocal.printDebug(["too many thirds"])
                if not sixthsGood2: environLocal.printDebug(["too many sixths"])
            except ModalCounterpointException:
                pass

        environLocal.printDebug(["GENERATING THIRD VOICE NOW"])
        goodHarmony3 = False
        goodMelody3 = False
        thirdsGood3 = False
        sixthsGood3 = False
...
def getRandomCF(mode = None):
    ...
    >>> from music21 import *
    >>> cf = counterpoint.counterpoint.getRandomCF()
    >>> cf.keys()
    ['notes', 'mode']
