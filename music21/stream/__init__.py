...
#        return max([g.priority for g in found])
    ...
    def getElementsByOffset(self, offsetStart, offsetEnd=None,
        ...
        for e in self.elements:
            ...
            elementEnd = common.optionalNumToFraction(offset + dur.quarterLength)
...
#             element = item
    ...
    def _uniqueOffsetsAndEndTimes(self, offsetsOnly = False,
        ...
        for e in self.elements:
            ...
            endTime = common.optionalNumToFraction(o + e.duration.quarterLength)
    ...
    def extendTies(self, ignoreRests=False, pitchAttr='nameWithOctave'):
        ...
        for i, e in enumerate(srcFlat._elements):
            ...
            connections = _getNextElements(srcFlat, i,
                e.getOffsetBySite(srcFlat, returnType='rational') + e.duration.quarterLength)
...
#             e.transpose(value, inPlace=True)
    ...
    def sliceByQuarterLengths(self, quarterLengthList, target=None,
        ...
        for e in eToProcess:
            ...
            if common.optionalNumToFraction(sum(quarterLengthList)) > e.quarterLength:
                continue
            elif not common.optionalNumToFraction(sum(quarterLengthList)) == e.quarterLength:
            ...
            if not common.optionalNumToFraction(sum(qlProcess)) == e.quarterLength:
            ...
            for eNew in post:
                ...
                oInsert = common.optionalNumToFraction(oInsert + eNew.quarterLength)
        ...
        for v in vGroups:
            for e in v.elements:
                        ...
                        if getOverlaps is True or e.offsetRational >= lastEnd:
                            if e.offsetRational >= lastEnd:  # is not an overlap...
                                ...
                                if hasattr(e, "duration"):
                                    lastEnd = common.optionalNumToFraction(lastStart + e.duration.quarterLength)
                                else:
                                    lastEnd = lastStart
                                lastWasNone = False
                                lastPitch = e.pitch
                ...
                # if we have a chord
                    ...
                    lastEnd = common.optionalNumToFraction(e.offsetRational + e.duration.quarterLength)
